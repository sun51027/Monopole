...
public:
  ...
  void doAnalysis(bool trg, const vector<MonoCandidate> & cands) {
	...
	PlotSet & z = nm1Plots_[0];
	z.GetPlot(FracSatVNstrips)->Fill(cand.subHits_,cand.subSatHits_/cand.subHits_);
	...
	z.GetPlot(ABCD)->Fill(cand.f51_,cand.dEdXSig_);
      ...
      // bool passHLT_Photon165_HE10_ = 0;
  	...
  	// fill the HLT plots
  	PlotSet & y = cutFlow_[0];
	y.GetPlot(FracSatVNstrips)->Fill(cand.subHits_,cand.subSatHits_/cand.subHits_);
...
private:
  ...
  // add a number to4U make plot names unique
...
const double MonoCutFlow::f51Cut_ = 0.85;
const double MonoCutFlow::dEdXSigCut_ = 10;
const double MonoCutFlow::f51LooseCut_ = 0.7;
...
int main ( int argc, char **argv ) {
 ...
 trigNameList.push_back("HLT_Photon200_v");
  // trigNameList.push_back("HLT_Photon165_HE10_v");
  ...
  // Bool_t passHLT_Photon165_HE10_;
  ...
  // tree->SetBranchAddress("passHLT_Photon165_HE10" , &passHLT_Photon165_HE10_);
  ...
  // std::cout<<passHLT_Photon175<<std::endl;
